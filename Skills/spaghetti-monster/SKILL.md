...
* [ ] The parcel-plan template is available at `docs/plans/template-plan.md` for Stage 2.
...
2. A backlog parcel plan per high-value target at `docs/plans/<code>-<slug>-plan.md`, with Phases 1 + 2 fully pre-populated.
3. An entry added to `docs/backlog/backlog-index.md` for each plan.
...
4. **Idempotency check:** for each kill-list target, search `docs/logs/agent-changelog.md` for refactor entries on the same file in the last 90 days. If found, surface the prior entry; the user decides whether to create a new plan.
...
2. **Create the plan file** at `docs/plans/<code>-<slug>-plan.md` using the canonical template at `docs/plans/template-plan.md`.
...
For the full execution pipeline, see the prior v3.1 SKILL.md archived in `docs/archive/`, or the `ptp-code-surgeon` skill which is the canonical execution persona.
...
5. **One plan per accepted target, no batching.** No plan that covers two unrelated targets.
 6. **Pre-populate Phases 1 AND 2** on every plan. Use `docs/plans/template-plan.md` as the canonical scaffold.
...
8. **Update `docs/backlog/backlog-index.md`** with a row per new plan.
