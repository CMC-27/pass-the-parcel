...
This skill owns **Group D: Execution & Verification (Phases 7-8)** of the `pass-the-parcel` pipeline. When activated as the `Executor`, you operate in a completely clean context window. Your single goal is to read the validated plan file at `docs/plans/[plan-name].md` and apply changes directly to the codebase without introducing regressions. **Phase 7 triggers ONLY after Gate C is cleared by explicit user input — never before.**
