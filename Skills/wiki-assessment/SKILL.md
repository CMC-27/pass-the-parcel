...
- `docs/backlog/backlog-index.md` (operational)

### C. Project Audit Logging (The Wrap-Up Protocol)
Whenever an audit is complete or a finding is remediated, append a chronological entry to `docs/logs/agent-changelog.md` using this exact format:
...
Before walking the 19 docs in order, you may optionally read `docs/logs/agent-changelog.md` to surface recently modified features. Use this to **prioritize** which slots to scrutinize first — it is **not** a substitute for the full per-doc loop. The full walk is always required regardless of what the changelog reveals.
