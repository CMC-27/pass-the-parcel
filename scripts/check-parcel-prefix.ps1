...
.SYNOPSIS
    ...
    (.devops/agents/parcel.agent.md + .devops/agents/ptp-*.subagent.md).
...
.DESCRIPTION
    ...
    (parcel.agent.md = selectable, ptp-*.subagent.md = subagents). Each file carries
    ...
    With -Sync:     rebuilds each agent file's prefix from base-context.md, preserving
                    the frontmatter and the agent-unique content.
...
$agentFiles = $agentFiles | Sort-Object Name
if (-not $agentFiles) { throw "No PREFIX-LOCKED agent files (parcel.agent.md / ptp-*.subagent.md) found in $agentsDir" }
