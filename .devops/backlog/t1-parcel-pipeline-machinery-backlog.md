...
| Code | Title | Status | Description | Plan |
|------|-------|--------|-------------|------|
| T1-E3.06 | Sprint 8 gate & preflight coverage gaps | QUEUED | Three deterministic coverage gaps surfaced while executing Sprint 8: `.devops/sprints/**` sits outside `check-utf8-agents.ps1`'s scope (queue plans lose encoding coverage); `@sprint-plan` never tests mutual `touches` overlap across the set it commits, so it can commit a queue the batch runner can only execute one wave at a time; and `spaghetti-monster-scan.cjs` crashes on a repo with no `src/` instead of no-oping, while never scanning the machinery roots (`scripts/`, `.devops/skills`, `.devops/agents`, `.devops/templates`). | [sprint-9](../sprints/sprint-9-batch-hardening/t1-e3.06-sprint-8-gate-preflight-gaps-plan.md) |
| T1-E3.07 | Positive-evidence auto-clear for AUTO gates | QUEUED | Replace the negative AUTO test ("no `REJECTED` line") with a positive evidence contract per gate, so an empty or vague self-review can no longer pass by omission; an unproven gate becomes a stop-the-line. Wave 1 of the batch-hardening sequence. | [sprint-9](../sprints/sprint-9-batch-hardening/t1-e3.07-positive-evidence-auto-clear-plan.md) |
| T1-E3.08 | Batched clarification questionnaire | QUEUED | Turn Phase 3's "one question at a time (non-negotiable)" into a batched-questionnaire mode (recommended defaults first, final validation still separate); interactive path only — the batch keeps auto-answering. Wave 2. | [sprint-9](../sprints/sprint-9-batch-hardening/t1-e3.08-batched-clarification-questionnaire-plan.md) |
