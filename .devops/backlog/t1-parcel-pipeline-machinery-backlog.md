...
| Code | Title | Status | Description | Plan |
|------|-------|--------|-------------|------|
| T1-E3.03 | `GATE_D_USER_APPROVAL` claim status + dependency predicate + fixpoint batch runner | SPRINT-8 | Codifies the Sprint 7 dependency ruling: a first-class "executed to Gate D" claim status (replacing dead `IN_PROGRESS`) satisfies `depends_on`, and the batch runner iterates to a fixpoint so intra-queue chains execute. Mirrored across the prefix lock + `plan-lifecycle`. | [plan](../sprints/sprint-8-sprint-lifecycle-hardening/t1-e3.03-gate-d-user-approval-plan.md) |
| T1-E3.04 | Batch sprint wrap-up for `parcel-sprint` | SPRINT-8 | Gives the batch host a coalesced wrap-up: confirm each executed plan, mark complete, archive the batch in one pass via a new batch scope on `@agent-wrap-up`; host may spawn `wiki-writer`. Depends on T1-E3.03. | [plan](../sprints/sprint-8-sprint-lifecycle-hardening/t1-e3.04-sprint-batch-wrap-up-plan.md) |
| T1-E3.05 | Retire the selectable `parcel-fast` orchestrator | SPRINT-8 | Parcel Fast becomes subagent-only: delete `.devops/agents/parcel-fast.agent.md`, remove its Model Registry + Orchestrator Presets + `opencode.json` entries, re-inline the prefix into the 9 remaining locked agents, and prune the orphan from satellites. | [plan](../sprints/sprint-8-sprint-lifecycle-hardening/t1-e3.05-retire-parcel-fast-orchestrator-plan.md) |
