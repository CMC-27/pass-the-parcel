...
- **[2026-09-23] A reversal sweep must match the concept, not one spelling** — the no-model contract's retirement check grepped `force-stamp` and declared the live prose clean, while `re-stamps`, `pre-bound`, `ship concrete` and `old binding behaviour` survived in `README.md`, `SATELLITE-BOOTSTRAP.md` and `MATURITY.md` (found a day later by a principle audit). *Do instead:* before declaring a reversal swept, enumerate the old mechanism's whole vocabulary — every verb, noun and idiom that could state it — grep them all over the live surface, and keep the lineage mentions (they are records, not regressions).

- **[2026-09-23] A parcel that adds a contract surface without a check must name the missing gate and park it in the same wrap-up** — `T1-E3.16` deliberately added the `stories:` front-matter row at the smallest surface (three prose files, no persona cascade, no `plan-lifecycle.md` edit) and left no deterministic check behind it; the consumer falls back to acceptance criteria, so a plan committed with no story and no recorded skip reason fails **invisibly**. The gap is real but was never stated as a limitation at the gate — it surfaced only at wrap-up, by reading the parcel's own Q1 ruling. *Do instead:* when a parcel ships an invariant without a check, say so in the Completion Note and park the check as a backlog item before archiving, so the honour-system link is a recorded decision rather than a silent one (here: `T1-E3.18`).

- **[2026-09-23] Overlapped queues serialize through the archive, not the batch — plan the wrap-up cadence, not the wave count** — Sprint 10's seven-plan queue collapsed to a 2-plan first batch: every remaining plan shared a file with one of the two committed runners, and `.devops/plans/` occupancy (not sprint.md's 3-wave forecast) is what the predicate reads. A fixpoint of zero eligible plans is a **normal terminal state**, not a stall. *Do instead:* size the batch loop to the overlap graph (queue-pairwise `touches` decides the first pass), then rely on the wrap-up archiving completed plans to un-block the next `@sprint-run` — the forecast's wave table is advisory, the script's live JSON is the contract.

- **[2026-09-20] A reversal is only honest when the gate that guarded the old invariant is moved, not deleted** — `T1-E1.04` deleted every hardcoded model binding, and the obvious "simplification" was to delete the ~150-line binding pass with it. `@Managed Simplicity` forbids that: *a gate that must go is moved, not removed*. The invariant still existed, inverted — so the check now asserts **absence** (`NOMODEL`) instead of agreement, and the sync strips instead of stamping. *Do instead:* before deleting a validation pass, ask what it proved; if that property is still required under the new design, invert the assertion and prove it non-vacuous with negative tests.
- **[2026-09-20] A doc-stated reversal must be recorded in three places, or it re-litigates itself** — the model-routing axis has now been decided three times (`.01` abstract slots → `.03` force-stamped registry → `T1-E1.04` no bindings), and each reversal cost a plan that first had to rediscover the previous one. *Do instead:* when reversing a recorded decision, write the **lineage** (what each prior position was and why it changed) into the knowledge register, not just the new position — the next reader must be able to see why *this* route differs from the one the objection killed.
