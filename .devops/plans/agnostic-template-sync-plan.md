# Plan: Agnostic Template — Pull-Based Sync + Machinery Versioning

**Status:** PLANNED (awaiting execution)
**Owner:** to be executed by an implementation agent (qwen-3.8-flash)
**Date drafted:** 2026-09-03
**Depends on:** nothing in flight, but see Conflict Constraints — an adjacent session owns parcel agent architecture.
...
re-design any decision marked **DECIDED** — they were settled with the human owner.
...
- `scripts/sync-architecture.ps1` — push-based sync. Reads `.devops/sync-manifest.yaml`
  (`portable_dirs`, `portable_skills`, `portable_files` — a simple `- item` YAML subset parser),
...
- `.devops/sync-manifest.yaml` — declares the portable surface. Currently 3 dirs, 12 skills, 3 files.
  Its `verify_commands:` key is **dead config** — the sync script never reads it (verification is
  hardcoded in the script).
- `.devops/skills/` — 30 skills, one folder each with `SKILL.md`. Only 7/30 have `version:` in
  frontmatter, in inconsistent formats (`"1.0"`, `"1.2"`, one with leftover `author: "Antigravity"`).
- `.devops/agents/parcel-*.md` — pure-body runbooks; frontmatter deliberately lives in
  `opencode.json` (see `.wiki/rules/frontmatter.md` "Convention Exemptions"). **Never add YAML
  frontmatter to these files** — it would break the byte-for-byte PREFIX-LOCKED cache contract.
...
- `.wiki/templates/` — exists but empty (`.gitkeep` + README).
- `.devops/skills/update-global-skills/` and `.devops/skills/update-workspace-skills/` — legacy
  skills syncing against `~/.gemini/config/skills` and a `Skills/` directory that no longer exists.
  Nothing else references them.
- `scripts/tmp-*` files exist from the adjacent session — do not touch or delete them.
...
### W1 — Machinery versioning backfill
...
- Normalize: skills that already carry `version: "1.0"` / `"1.2"` → integer `1` (no history worth
  preserving). Remove any `author:` frontmatter (e.g. `agent-wrap-up`).
- Preserve every other frontmatter field exactly (especially multi-line `description:` values —
...
### W3 ordering note — the versioning consumer is W2; land W2 and W3 together.

### W2 — `scripts/pull-architecture.ps1` (new file, satellite-side)
...
2. If `-Source` looks like a git URL (`https://`, `git@`, ends with `.git`, or contains `://`):
   ensure cache dir `$env:USERPROFILE\.ptp\template` exists — `git clone --depth 1 <url>` into it,
...
4. Write `.ptp-source` in the satellite root with the resolved source (only when `-Source` was
   ...
   gitignore nothing don't get noise — acceptable either way).
...
### W3 — Version-aware pre-flight in `scripts/sync-architecture.ps1` (edit existing)
...
| `SOURCE-ABSENT` | manifest lists it, source missing (manifest bug — always report) |
...
- For skills: parse `version:` (integer) from source and target `SKILL.md` frontmatter via regex
  ...
  → treat as `0` and let DRIFT classification handle it.
- For dirs and standalone files: derive from `machinery-version` in the source manifest — if the
...
entirely, so nothing needs excluding once W5 lands — but keep the mechanism; if W5 has not yet
run when you test, temporarily exclude the two legacy skills).
3b. Remove `verify_commands` from the manifest (dead config) — the script's hardcoded
...
3c. Add `machinery-version` to the manifest parser and to the copy step: bump-report — after a
real (non-check, non-dryrun) sync, print `machinery-version: <N> materialised`.

### W4 — `@sync-architecture` skill (new: `.devops/skills/sync-architecture/SKILL.md`)
...
1. Determine the workspace root (cwd; refuse if no `.devops/` — wrong kind of workspace).
...
3. Interpret and summarize the verdict table in plain language; for `DRIFT` items, explicitly
   warn "locally customized copy — do not blindly overwrite; ask the user" and offer to open the
...
### W5 — Retire legacy `.gemini` skills
...
### W6 — Expand the portable skill surface via derivation (no list edits needed)

With W3's `excluded_skills` mechanism, the portable surface automatically becomes every skill in
the repo minus the two deleted legacy ones (≈ 29 skills incl. the new `sync-architecture`).
...
### W7 — Manifest + script portability fixups
...
7a. Add `scripts/wiki_lint.py` and `scripts/pull-architecture.ps1` to `portable_files`.
    ...
    `.wiki/` structure generically today — expect none or trivial).
...
### W8 — Seed templates (new dir `.devops/templates/`)
...
- `AGENTS.template.md` — derived from the repo's own `AGENTS.md` structure: mandatory-reading
  ...
  rules 1–4 are app-specific; rules 5–8 are machinery and can stay verbatim).
- `opencode.template.json` — derived from the repo's `opencode.json`: `instructions`/`skills.paths`
...
- `base-context.template.md` — skeleton matching the PREFIX-LOCKED contract: the cache-anchor
...
- `SATELLITE-BOOTSTRAP.md` — the checklist tying it all together:
  1. `powershell -NoProfile -File <path-to-template>\scripts\sync-architecture.ps1 -Target <this repo>`
     (one-time push bootstrap — pull script doesn't exist here yet), or
  ...
  4. Verify: `powershell -NoProfile -File scripts\pull-architecture.ps1 -Check` → IN SYNC.

### W9 — Docs

9a. `HOW-TO.md`: new section **"6. Syncing Machinery into a Satellite Workspace"** — the two
...
9e. `.devops/skills/agent-wrap-up/SKILL.md`: add one step to its workflow — for each modified
...
1. W1 (stamps) + W2 (pull script) + W3 (sync -Check + derivation) + W5 (retire legacy) — the core;
   W1/W3 must land together (versions without a consumer are decoration).
2. W4 (skill) — depends on W2.
3. W7 (manifest fixups) — depends on W3/W5.
4. W8 (templates) — independent, any time after W3.
5. W9 (docs) — last, once behavior is final.
...
#    from the satellite → expect UPGRADE for that skill and exit 1.
# 4. Revert the source skill edit (git checkout -- .devops/skills/<slug>/SKILL.md).
# 5. -DryRun path of pull-architecture.ps1 against the satellite → prints, writes nothing (verify via git -C satellite status).
