...
3. **Informed run preview before the single yes/no.** Once preflight passes, present the computed preview and take **one** yes/no:
   ...
   - the **lane readout** from the same JSON (`lanes`, `reserved_surfaces`): serial vs lane-B eligible, presentable as a **classification** — this loop is trunk-sequential, so both lanes execute serially and lane B may legitimately be empty (`.devops/rules/plan-lifecycle.md` § Claim Protocol → *Reserved Surfaces & the Lane Model*).
...
- **The counter is the batch wrap-up's, not yours and not a runner's.** `machinery-version` has one writer per batch — the follow-up batch wrap-up, reading the live value at that moment (`.devops/rules/plan-lifecycle.md` § Claim Protocol → *Counter Ownership*). Never bump it inside the loop. The prefix regenerate is the exception: it stays with the plan that edited a reserved prefix surface, and your own invariant is that `check-parcel-prefix.ps1` exits `0` at every claim boundary (`git worktree` lanes do **not** exist here: deviation 3 is unchanged).
