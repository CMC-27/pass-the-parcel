...
3. **Informed run preview before the single yes/no.** Once preflight passes, present the computed preview and take **one** yes/no:
   ...
   - the **lane readout** from the same JSON (`lanes`, `reserved_surfaces`): serial vs lane-B eligible, presentable as a **classification** — every path runs in place serially and lane B may legitimately be empty (`.devops/rules/plan-lifecycle.md` § Claim Protocol → *Reserved Surfaces & the Lane Model*).
...
- **Never claim past a `DEFERRED-MANUAL` hole.** A deferred plan is a pause at its slot, not a skip: the loop stops there and the report names the dependents the deferral strands. The yield is a **halt**, not a fourth deviation — `.devops/rules/plan-lifecycle.md` § Deviations stays at three.
- You write no implementation code: every edit is made by the spawned `ptp-parcel-fast`.
- **The counter is the batch wrap-up's, not yours and not a runner's.** `machinery-version` has one writer per batch — the follow-up batch wrap-up, reading the live value at that moment (`.devops/rules/plan-lifecycle.md` § Claim Protocol → *Counter Ownership*). Never bump it inside the loop. The prefix regenerate is the exception: it stays with the plan that edited a reserved prefix surface, and your own invariant is that `check-parcel-prefix.ps1` exits `0` at every claim boundary (no worktree, no plan branch — every path runs in place).
