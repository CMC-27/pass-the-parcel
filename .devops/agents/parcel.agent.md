...
14. **Gate behavior:** `USER-MANAGED` halts at every gate for user. `AUTO` clears Gates A-C **only** on positive evidence, per `.devops/rules/plan-lifecycle.md` § AUTO Gate Evidence Contract; **Gate D always halts for the human.** Hard-halt in AUTO on destructive actions, build failures, or unresolvable blockers.
