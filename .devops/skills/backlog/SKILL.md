...
> **Boundary:** This skill creates backlog items only. It does not scope, plan, or execute. **Parked** plans live in `.devops/backlog/<slug>-backlog.md`; pick-up renames the file to `.devops/plans/<slug>-plan.md` (the `git mv` is the in-flight signal). Status is tracked in each plan's State Dashboard — `BACKLOG` means parked, `PHASE_1`+ means in flight. See the `pass-the-parcel` skill's [Plan State Lifecycle table] for the canonical state machine.
...
The plan filename follows: `T{theme}-E{epic}.{impl}-{descriptive-slug}-backlog.md`

Example: `T3-E1.01-import-modal-horizontal-scroll-backlog.md`
...
3. **Determine** the next available implementation number. Scan `.devops/backlog/` for existing plans with the same `T{n}-E{n}.` prefix to find the highest existing number, then increment by 1.
...
2. A full early-prepared plan file at `.devops/backlog/{code}-{slug}-backlog.md` using the canonical Pass-the-Parcel template (full scaffold: Phases 1-10 + Wrap Up). Per the [Lifecycle table]:
   - **Status** → `BACKLOG` (parked, not in flight)
   - **Active Persona** → `Planner` (prepares the scaffold)
   - **File suffix** → `-backlog.md` (parked plans use this suffix)
   - **Directory** → `.devops/backlog/` (parked plans live here; pick-up `git mv`s to `.devops/plans/`)
...
- Add a table entry under the correct Theme > Epic section linking to `.devops/backlog/{code}-{slug}-backlog.md`.
