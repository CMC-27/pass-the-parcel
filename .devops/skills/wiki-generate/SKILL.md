...
1. Translate each `type:`-bearing concept doc to the local `format-version: 1` schema as a `status: in-progress` draft, record provenance (`okf_version: "0.2"`), and register the doc in its area spoke index. Never overwrite an existing target path.
...
> **The mechanical bridge was retired in T1-E4.01.** `scripts/wiki_okf.py` had no consumer left, so import/export is now a prose mapping done by the agent above, not a shipped CLI. `validate.yml` still asserts that every `.wiki/**/*.md` frontmatter block parses as YAML — the assertion survived the tooling.
