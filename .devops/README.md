...
| `.devops/agents/` | machinery | VS Code custom agents: `parcel.agent.md` + `parcel-sprint.agent.md` (orchestrators; `parcel-sprint` is the locked batch host) + `wiki-writer.agent.md` (selectable; `wiki-writer` is also subagent-invocable), `ptp-*.subagent.md` + `wiki-verifier.subagent.md` (subagents; `ptp-parcel-fast` is the hidden per-plan fast runner, spawned only by `parcel-sprint`) |
...
- **Parcel feedback (satellite → template):** the pathway for a satellite agent to route issues/improvements **upstream** to this repo's backlog (`@sync-architecture` skill step 5, "Report parcel feedback upstream"): the satellite drafts one parked-plan-shaped item per finding into a local `feedback/` outbox — no `gh`, no credentials, works offline. The operator hand-carries the items at the next sync: copy each as `.devops/backlog/<code>-<slug>-backlog.md`, register it in the owning theme register + Triage Panel via `@backlog`, delete the outbox item. Items arrive pre-triaged in shape (Findings with evidence, What-the-fix-does, Open question) so intake is copy-only triage, never re-authoring.
