...
## Earlier releases
- **v0.7.15** (2026-09-15) — Parcel Fast becomes **subagent-only**: the selectable `parcel-fast` orchestrator is retired along with every derived surface (agent file, Model Registry row, Orchestrator Presets row, `opencode.json` + seed entries), because "Parcel Fast" existed twice — one prefix apart from the hidden batch subagent — and the collision was a documented confusion hazard. The `/parcel-fast` shortcut is a deliberate loss: interactive fast runs are `parcel` + `AUTO`/`SINGLE` at plan start; unattended runs stay `parcel-sprint` → `ptp-parcel-fast`. The orphan is added to `prune_files` so an already-synced satellite drops it. machinery-version: 45 (44 at plan close; the wrap-up folded a fold-target annotation into .devops/rules/process-lessons.md, a synced rules file).
- **v0.7.14** (2026-09-14) — Knowledge Capture gets a **second destination**: machinery, process and tooling lessons route to a new `.devops/rules/process-lessons.md` register instead of piling up in the app-domain KC log — a one-way ratchet, because the wiki documents the app and can never absorb them. Consolidation gains a `promote-machinery` path (Phase 6b) and switches its size trigger from physical lines to a **25-entry ceiling**; `@sprint-close` reviews the register; `@knowledge-capture` gains subject routing and a one-entry-per-session budget. machinery-version: 43.
