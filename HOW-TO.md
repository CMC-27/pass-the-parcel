...
> **Binding note (opencode.json).** The `agent` block is **required** — a missing or empty block fails `check-parcel-prefix.ps1`. Model values are template-owned: the seed ships them concrete and `@sync-architecture` force-stamps the registry, the agent frontmatter and these values on every sync, so a satellite-side rebind is reverted by the next sync (see `.devops/skills/model-routing/SKILL.md` §3).
...
### Model binding
The parcel pipeline routes by capability class — `orchestration`, `retrieval/inventory`, `retrieval/Q&A`, `deep planning/authoring`, `adversarial review`, `product review`, `execution`, `independent audit` — and prose never names a vendor model. The Model Registry in `.opencode/plans/base-context.md` is the **single source**: the agent frontmatter `model:` lines and every `opencode.json` `agent.<key>.model` are derived, and `@sync-architecture` force-stamps all three surfaces into a satellite on each sync — rebind in the template (registry + seed mirror, then `check-parcel-prefix.ps1 -Sync`), not in the satellite. `scripts/check-parcel-prefix.ps1` proves the live registry, the seed registry, all binding-file frontmatter, `opencode.json` and the seed opencode config agree. CI (`.github/workflows/validate.yml`) enforces the prefix, encoding, wiki, JSON, SelfTest, and coverage gates on every push **in this template repo**; a satellite wires the same gates into its own workflow (the checks ride in the synced `scripts/check-parcel-prefix.ps1`). `scripts/sync-architecture.ps1 -SelfTest` smoke-tests the transport engine — including binding propagation — on `ubuntu-latest`.
