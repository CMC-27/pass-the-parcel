...
4. **Respect the governance layer.** Apply `.wiki/rules/` before creating/moving/renaming/linking anything and `.wiki/rules/language/` before writing anything.
5. **Use the wiki first.** Before searching the codebase, read the wiki index (`.wiki/core/00-system-index.md`) and the relevant spoke index. Subagents must be told the same.
