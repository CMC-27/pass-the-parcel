...
>
> **Note (2026-09-13, revised 2026-09-18):** this run predates the claim protocol. Pickup is now backlog → sprint queue → claim (`git mv` into `.devops/plans/` + `claim: <code>` commit, in place on the trunk — no worktree, one claim at a time), and the pipeline statuses begin at `QUEUED`/`CLAIMED`. The ten phases, gates, and revision loop shown below are unchanged.
