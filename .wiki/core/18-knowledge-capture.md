...
- **`@sprint-run` batch deviations**: a sprint's committed queue may be run unattended by the `parcel-sprint` host, which relaxes three rules and no more. **trunk-sequential** — the batch keeps the `git mv` + `claim: <code>` commit but **drops `git worktree add`**, because one host runs the queue serially (never claim two worktrees from a serial runner). **Batched Gate D** — each plan terminates at `PHASE_9` with Gate D `OPEN` and one human verdict covers the whole batch; deferred, never skipped. **Strict Context Isolation exception** — one plan's Phases 1→9 run in a single fresh per-plan context; the one-phase-group-per-session bound stands for every other run. *(2026-09-13)*
