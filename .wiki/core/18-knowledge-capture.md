...
- **A force-stamped projection needs an insert path, not just a rewrite path**: a sync that rewrites only the config keys a target already carries silently fails to propagate template-side *growth* — a new registry key never arrives and the target's own gate fails (`no Model Registry row`). *Do instead:* when force-stamping a projection, insert absent keys too, and test against a target that predates the new key.
...
- **Transport completeness** (F8/F9, machinery v41): the sync must materialise every reference it ships *and* repair every derived surface it owns. A path named by the shared prefix or a shipped skill is portable — the canonical plan scaffold `.devops/plans/template-plan.md` joins `portable_files` because the prefix and five skills name it. A binding surface the template owns is repaired for keys the satellite has **never authored**: a registry key missing from the target's `opencode.json` `agent` block is inserted whole from the target's own synced seed, so a newly shipped agent arrives runnable — while an entry the satellite authored is never restructured (only its `model` value is stamped). This supersedes the `ponytail:` ceiling recorded in `T1-E1.03`, which named exactly this upgrade path. *(2026-09-13)*
...
### Agents & Models
- **Inherited model routing — no agent declares a model** (supersedes *Registry-canonical model binding*, 2026-09-13, which superseded *Dual-surface agent binding*, 2026-09-06, and reversed T1-E1.01's abstract slots; this entry is the **third** position on that axis): **no binding surface declares a model.** Not agent frontmatter, not `opencode.json` or its seed, not the registry table. Every agent runs on **the model selected in the CLI / picker**. Where a run spawns subagents the **operator chooses at run time** — **per gate** (A/B/C/D) for `parcel` in `MULTI`, or **one for the whole batch** under `@sprint-run` — recorded in the plan's frozen `PlanSettings.Models` row (the batch host hands the value down; the spawned runner records it, never chooses). The `## Model Registry` in `.opencode/plans/base-context.md` survives as a **capability-class reference** only: two cells per row, feeding the run-time question's recommendation. `check-parcel-prefix.ps1` asserts the invariant by **absence** (`NOMODEL` per clean surface; a `model:` line, an `agent.<key>.model`, or a third registry cell is a FAIL) while keeping key↔file coverage and seed parity; `sync-architecture.ps1` **strips** any model it finds, JSON-validated and reverted rather than left unparsable. **Runtime capability is asymmetric and must not be blurred:** VS Code's `runSubagent` accepts a spawn-time `model` (1.116.0, vscode#298380 closed); opencode does not (**#6651** + PR #11377 open) and the run **halts with the limitation named** rather than substituting silently — pre-bound variant agents were rejected because they reintroduce the hardcoding this removes. Lineage matters: `.01` relocated the binding machinery and it kept growing; `.03` made the registry a force-stamped single source; this revision **deletes** it. The complexity objection is answered by removal, not by a new home. *(2026-09-20)*
