...
### 1. Start Here: `wiki/core/00-system-index.md`
...
**Read `wiki/core/08-state-context.md` FIRST.**
...
- Look up the feature-specific context in **`wiki/features/`** or check the physical layout of the codebase via **`wiki/core/06-directory-structure.md`**. It holds the exact file paths and dependencies.
...
| Writing a database query / API request | [Database Index](wiki/database/database-index.md) | Specific schema / endpoint doc |
| Editing overall layout or workspace shell | [App Shell Structure](wiki/core/07-app-structure.md) | Core layout component docs |
| Understanding state shapes / context | [State & Context](wiki/core/08-state-context.md) | State management docs |
...
6. **Mandatory Wrap-Up Protocol:** Whenever a task or feature is complete — including when the user says anything like "wrap up", "we're done", "ship it", "that's it", or closes out a conversation — you **MUST** perform the following steps:
   ...
   **Part 1 - Audit Logging:** Document your actions by adding a row to `dev/logs/agent-changelog.md`:
   ```markdown
   ...
   **Part 3 — Archiving Completed Plans:** Move the completed plan file from `dev/plans/[plan-name].md` to `dev/archive-plans/[plan-name].md`.
